...
class GATv2Conv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Learnable temperature per head for attention sharpness control
        # tau < 1 -> sharper/sparser attention, tau > 1 -> softer/dense attention
        # tau = 1 recovers standard GATv2
        self.temperature = Parameter(torch.ones(1, heads, 1))
    ...
    def reset_parameters(self):
        ...
        glorot(self.att)
        zeros(self.bias)
        # Initialize temperature to 1.0 (standard GATv2 behavior)
        torch.nn.init.ones_(self.temperature)
    ...
    def edge_update(self, x_j: Tensor, x_i: Tensor, edge_attr: OptTensor,
        ...
        # Temperature-gated attention: scale logits before softmax
        # tau < 1 sharpens attention (sparser), tau > 1 flattens (denser)
        alpha = alpha / self.temperature.view(1, -1)
    ...
    def __repr__(self) -> str:
        return (f'{self.__class__.__name__}({self.in_channels}, '
                ...
                f'temp={self.temperature.data.squeeze().tolist()})')
    
    def attention_entropy(self, alpha: Tensor, index: Tensor, 
                          ptr: OptTensor = None, dim_size: Optional[int] = None) -> Tensor:
        """
        Compute entropy of attention distributions per node per head.
        Returns: [num_nodes, heads] entropy values.
        Lower entropy = sparser attention.
        """
        from torch_geometric.utils import softmax
        eps = 1e-9
        # alpha is already normalized, compute -sum(p * log p) per segment
        ent = -(alpha * (alpha + eps).log()).sum(dim=-1)  # [E, H]
        # Now segment-reduce by target node
        if ptr is not None:
            num_nodes = ptr.size(0) - 1
            out = torch.zeros(num_nodes, self.heads, device=alpha.device)
            for i in range(num_nodes):
                start, end = ptr[i].item(), ptr[i+1].item()
                if start < end:
                    out[i] = ent[start:end].sum(dim=0)
        else:
